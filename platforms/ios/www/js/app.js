...
/*Data pour le graph en forme de ligne*/
 ...
 $scope.showSublist = function(idx) {
  ...
  $scope.edit = function(idx) {
    var dernierPoids = 0;
    if($scope.tasks[idx].journal.length>0) {
      dernierPoids = $scope.tasks[idx].journal[$scope.tasks[idx].journal.length - 1].poids;
    }
    $scope.data = {response: dernierPoids};
    $ionicPopup.prompt({
      title: "Ajout d'un poids",
      inputType:"number",
      inputPlaceholder: dernierPoids,
      scope: $scope
      }).then(function(res){
        console.log('le poids est ', res);
                $scope.tasks[idx].journal.push({date:new Date(), poids:$scope.data.response});
